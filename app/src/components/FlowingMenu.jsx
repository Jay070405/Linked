...
function MenuItem({ link, text, image, speed, textColor, marqueeBgColor, marqueeTextColor, borderColor }) {
  ...
  useEffect(() => {
    const calculateRepetitions = () => {
      ...
      // A native dialog is initially hidden. Re-measure when it becomes visible.
      if (!Number.isFinite(contentWidth) || contentWidth < 1) return;
      ...
      setRepetitions(Math.min(32, Math.max(4, needed)));
    };
    ...
    return () => {resizeObserver.disconnect();window.removeEventListener('resize', calculateRepetitions);};
  }, [text, image]);

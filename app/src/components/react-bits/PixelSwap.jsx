...
function PixelSwap({
  ...
  useEffect(() => {
    ...
    const finish = () => {
      containerRef.current?.dispatchEvent(new CustomEvent('vibe:pixel-finish', {bubbles:true, detail:{active:to}}));
      stopAnimations();
      setShownActive(to);
      setTransition(null);
    ...
    if (!source || !frozenGrid.pixels.length || reduced || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      finish();
      return;
    }
    ...
    // Share the original grid and timing with the curved GPU surface.
    containerRef.current?.dispatchEvent(new CustomEvent('vibe:pixel-start', {bubbles:true, detail:{active:to, grid:frozenGrid, total, pixelMs, pixelScale:settings.pixelScale}}));
    if (textureMode) {
    ...
    timerRef.current = window.setTimeout(finish, total);
    return stopAnimations;
  }, [stopAnimations, transition, textureMode, reduced]);

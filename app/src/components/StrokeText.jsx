...
// React Bits official registry source, adapted for inherited editorial typography.
// Additions: natural layoutBox/baseline, font family/style, pause/reduce, completion.
...
const StrokeText = ({
  ...
  const fontStyle = useMemo(
    ...
    [fontSize, fontWeight, letterSpacing, fontFamily, textFontStyle]
  );
  ...
  }, [characters, fontSize, fontWeight, letterSpacing, strokeWidth, fontFamily, textFontStyle]);

  useEffect(() => {
    if (timelineRef.current) timelineRef.current.paused(paused);
  }, [paused]);
  ...
  }, [box, dash, drawDuration, fillDelay, stagger, ease, trigger, fillMode, reverse, reduced, delay]);

  const viewBox = layoutBox ? `0 ${-layoutBox.baseline} ${layoutBox.width} ${layoutBox.height}` : box ? `${box.x} ${box.y} ${box.width} ${box.height}` : `0 ${-fontSize} 600 ${fontSize * 1.3}`;

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${layoutBox ? 'stroke-text--natural' : ''} ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()}
      style={{ ...style, '--stroke-text-height': `${layoutBox?.height || Math.round(fontSize * 1.3)}px` }}
      role="img"
      data-paused={paused ? 'true' : 'false'}
      aria-label={String(text ?? '')}
    >
      <svg className="stroke-text__svg" viewBox={viewBox} preserveAspectRatio={layoutBox ? 'none' : 'xMidYMid meet'} aria-hidden="true">
        {fillMode === 'wipe' && box && (
          <defs>

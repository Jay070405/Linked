...
export default function RocoCase({lang='zh',reduced=false,onNavigate}) {
 ...
 return <article ref={page} className={`rco-page rco-case-page${reduced?' rco-reduced':''}`} data-nav-tone="light">
  <header className="rco-case-header rco-width"><div className="rco-case-meta"><span>ROCO KINGDOM: WORLD / SYSTEMS DESIGN</span><span>{en?'Personal study · Original proposal':'个人系统研究 · 原创提案'}</span></div><div className="rco-case-title"><h1><ExpressiveTitle reduced={reduced}>{en?<>From waiting<br/>to <em>choosing.</em></>:<>从等待结果，<br/>到<em>规划路线。</em></>}</ExpressiveTitle></h1><div><h2>{en?'Shiny encounters, examined.':'洛克王国：世界 · 异色系统'}</h2><p>{en?'S3 gameplay, configuration and resource loops; two original S5 directions.':'S3玩法、配置与资源循环，以及S5双方案提案。'}</p><p className="rco-fine">{en?'Synced with Lark · 17 Sep 2026':'飞书内容同步 · 2026.09.17'}</p><div className="rco-case-top-links"><a href="/assets/docs/roco-system-design-portfolio-zh.pdf" target="_blank" rel="noreferrer">{en?'Open PDF':'查看 PDF'} ↗</a><button type="button" onClick={()=>jump('roco-s5')}>{en?'Compare the proposals':'查看双方案'} ↓</button><button type="button" onClick={()=>onNavigate?.('/systems/roco/model')}>{en?'Open the model':'打开模型'} ↗</button></div></div></div><figure className="rco-cover"><img src="/assets/projects/roco-cover.png" alt={en?'Personal S5 forest concept cover with fantasy creatures':'个人 S5 森林概念封面与幻想精灵'}/><figcaption><span>01 / CONCEPT COVER</span><span>{en?'Personal S5 concept · Unofficial · AI-assisted cover':'个人 S5 概念 · 非官方 · AI 辅助封面'}</span></figcaption></figure></header>

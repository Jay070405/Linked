...
const LeagueCase=lazy(()=>import('./LeagueCase'));
...
export default function PortfolioPages({page,lang,reduced,onNavigate,onBack,pageScroll,archiveState}){
 ...
 useEffect(()=>{
   const suffix=page.type==='vibecoding'?'Vibe coding':page.type==='field01'?'FIELD / 01':page.type==='desk'?'Visual Worlds':page.type==='archive'?(en?'Work archive':'作品档案'):page.type==='art'?(en?page.item.titleEn:page.item.title):page.type==='model'?(en?'Roco · Model lab':'洛克 · 模型实验室'):page.item?.id==='league-of-legends'?(en?'League of Legends · Live operations':'英雄联盟 · 长线运营'):(en?'Roco Kingdom · Case study':'洛克王国：世界 · 系统策划');
 ...
 return <><main ref={viewport} className={'portfolio-page-viewport route-'+page.type} id="portfolio-page" tabIndex={-1} inert={capture||cameraActive?true:undefined} aria-hidden={capture||cameraActive?true:undefined} data-nav-tone={['desk','archive'].includes(page.type)?'dark':'light'} aria-label={en?'Portfolio page':'作品集页面'} onScroll={event=>{pageScroll.current.set(path,event.currentTarget.scrollTop);}}>
   <PageBoundary key={path} lang={lang}><Suspense fallback={<div className="portfolio-page-loading" role="status"><span>JAY LIN / PORTFOLIO</span><p>{en?'Unfolding the work…':'正在展开作品…'}</p></div>}>
    {page.type==='vibecoding'?<VibeCoding {...props}/>:cameraActive?null:page.type==='desk'?<ArtDesk {...props}/>:page.type==='archive'?<WorkArchive {...props} archiveState={archiveState}/>:page.type==='art'?<WorkDetail {...props} item={page.item}/>:page.type==='model'?<RocoModel {...props}/>:page.item?.id==='league-of-legends'?<LeagueCase {...props}/>:<RocoCase {...props}/>}
  ...
  </div>}{capture&&<SharedProjectTransition capture={capture} hostRef={cameraHost} galleryRef={viewport} onFinish={finishProject} onCancel={cancelProject} ready={cameraReady} lang={lang}/>}{!cameraActive&&!capture&&<button className={'portfolio-return'+(['desk','archive'].includes(page.type)?' is-dark':'')} onClick={onBack} aria-label={en?'Return to previous page':'返回上一页'}>← <span>{en?'Back':'返回'}</span></button>}</>;
}

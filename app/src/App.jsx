...
function Finale({lang,reduced,blossomProgress,onAbout}) {return <section className="finale-journey" id="practice" data-nav-tone={reduced?'dark':'light'}><div className="finale-stage"><div className="finale-ground"/><div className="blossom-wrap"><BlossomScene getProgress={()=>blossomProgress.current} reducedMotion={reduced}/></div><div className="blossom-small-copy"><span className="eyebrow">A SMALL IDEA. AN ENTIRE WORLD.</span><p>{lang==='en'?'Keep going. Let it bloom.':'继续向下，让想法盛放。'}</p></div><div className="bloom-title"><span><ExpressiveTitle reduced={reduced}>{lang==='en'?'MAKE ROOM':'让想象'}</ExpressiveTitle></span><span><ExpressiveTitle reduced={reduced}><i>{lang==='en'?'for':'自由'}</i> {lang==='en'?'WONDER.':'生长。'}</ExpressiveTitle></span></div><div className="philosophy"><span className="eyebrow">03 / PHILOSOPHY & PRACTICE</span><p className="reading-text">{(lang==='en'?enPhilosophy:zhPhilosophy).split(lang==='en'?' ':'').map((c,i)=><span key={i}>{c}{lang==='en'?' ':''}</span>)}</p><div className="philosophy-details"><p>{lang==='en'?'Systems thinking gives a world its logic. Art gives it a reason to be remembered.':'以系统思维构建世界的逻辑，\n以视觉创作留下值得记住的瞬间。'}</p><button onClick={onAbout} className="text-link">{lang==='en'?'Meet the person behind it ↗':'认识我 ↗'}</button></div></div><div className="philosophy-ribbon" aria-hidden="true"><span>WORLDBUILDING · NARRATIVE · SYSTEM DESIGN · VISUAL DEVELOPMENT · WORLDBUILDING · NARRATIVE · SYSTEM DESIGN · VISUAL DEVELOPMENT ·</span></div><div className="return-copy"><i>Back to a beginning.</i><span>{lang==='en'?'Every ending is somewhere to start.':'每个结束，都可以是下一幕的开始。'}</span></div></div></section>;}
